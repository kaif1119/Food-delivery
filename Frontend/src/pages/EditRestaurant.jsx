...
const EditRestaurant = () => {
  ...
  const { currentRestaurant, isLoading, error, updateSuccess } = useSelector(
  ...
  // Fetch restaurant details if not present or different ID
  useEffect(() => {
  ...
  // Pre-populate form when currentRestaurant is loaded
  useEffect(() => {
    if (currentRestaurant && currentRestaurant._id === id) {
      const timer = setTimeout(() => {
        setFormData({
          ...
          deliveryTimeInMinutes: currentRestaurant.deliveryTimeInMinutes || 30,
          cuisineInput: currentRestaurant.cuisine ? currentRestaurant.cuisine.join(', ') : '',
          isOpen: currentRestaurant.isOpen !== undefined ? currentRestaurant.isOpen : true,
        });
      }, 0);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const { name, description, address, city, image, deliveryTimeInMinutes, cuisineInput, isOpen } = formData;
    ...
    dispatch(updateRestaurant({ id, restaurantData }));
  };

  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      {isLoading && <Loader message="Updating restaurant details..." />}

      {/* Back to Dashboard */}
      <div className="mb-6">
        <button
          onClick={() => navigate('/dashboard')}
          className="inline-flex items-center gap-1.5 text-stone-500 hover:text-stone-850 font-semibold text-sm transition-all cursor-pointer"
        >
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-stone-500 text-sm max-w-md mx-auto">
          Keep your restaurant details up to date for your hungry Trailblazers
        </p>
      </div>
      ...
      <div className="bg-white rounded-3xl p-8 sm:p-10 shadow-xl border border-stone-200/50">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2 bg-stone-50 p-4 rounded-2xl border border-stone-200/60 flex items-center justify-between select-none">
              <div>
                <label className="block text-xs font-bold text-stone-700 uppercase">Restaurant Status</label>
                <span className="text-[11px] text-stone-450">Toggle whether your restaurant is open to accept orders.</span>
              </div>
