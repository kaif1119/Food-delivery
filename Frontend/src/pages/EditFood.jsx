...
import { fetchFoodById, updateFood, resetFoodUpdateSuccess, clearFoodError } from '../features/foods/foodSlice';
...
const EditFood = () => {
  ...
  const { currentFood, isLoading, error, updateSuccess } = useSelector((state) => state.foods);

  // Fetch food details on mount
  useEffect(() => {
    dispatch(clearFoodError());
    dispatch(resetFoodUpdateSuccess());
    dispatch(fetchFoodById(id));
  }, [dispatch, id]);

  // Populate form data once currentFood is loaded
  useEffect(() => {
    if (currentFood && currentFood._id === id) {
      const timer = setTimeout(() => {
        setFormData({
          ...
          isVeg: currentFood.isVeg !== undefined ? currentFood.isVeg : true,
          isAvailable: currentFood.isAvailable !== undefined ? currentFood.isAvailable : true,
          preparationTimeInMinutes: currentFood.preparationTimeInMinutes || 20,
        });
      }, 0);
  ...
  useEffect(() => {
    if (updateSuccess && currentFood) {
      const restaurantId = typeof currentFood.restaurantId === 'object'
        ? currentFood.restaurantId?._id
        : currentFood.restaurantId;

      dispatch(resetFoodUpdateSuccess());
      navigate(`/restaurants/${restaurantId}`);
    }
  }, [updateSuccess, navigate, dispatch, currentFood]);
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(updateFood({ id, foodData }));
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      {isLoading && <Loader message="Updating food item..." />}
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-stone-500 text-sm max-w-md mx-auto">
          Keep your menu details up to date and accurate for your customers
        </p>
      </div>
